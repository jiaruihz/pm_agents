...
class UnifiedWeatherThetaNoStrategy(IStrategy):
    ...
    async def init(self, context: StrategyContext) -> None:
        ...
        # Initialize token states in context
    ...
    async def on_market_tick(
        ...
        # In generic engine, we get position from context.positions instead of quote_input
        ...
        # Pull all strategy parameters with safe defaults
        ...
        # -------------------------------------------------------------
        # Entry Logic (If no position yet)
        # -------------------------------------------------------------
        if position <= 1e-9:
            ...
            # Basic entry size logic using a budget mock for the unified engine context
            ...
            logger.info(f"[{self.key}] Entry conditions met for {token_id}. Issuing BUY command.")
            return [
                OrderCommand(
                    ...
                    side=OrderSide.BUY,
                    size=buy_size,
                    price=round(entry_price, 3), # Simplistic quantize
                    order_type=OrderType.LIMIT
                )
            ]

        # -------------------------------------------------------------
        # Exit Logic (If currently holding)
        # -------------------------------------------------------------
        ...
        if stop_loss_hit or take_profit_hit or max_hold_hit or pre_settlement_exit or range_break_hit:
            ...
            logger.info(f"[{self.key}] Exit conditions met for {token_id}. Issuing SELL command.")
            return [
                OrderCommand(
                    instance_id=context.instance_id,
                    strategy_key=self.key,
    ...
    def _sync_state(self, state: TokenState, position: float, mid: float, now: float) -> None:
        """Keep token state synced with actual positions to calculate entry benchmarks."""
    ...
    async def on_order_update(
        ...
        """Hook order fills to update our runtime context position mock."""
        if event.status == OrderStatus.FILLED:
            # Simplistic position update based on internal command history
            # In a real environment, the engine / sync service will update context.positions
            logger.info(f"[{self.key}] Order {event.order_id} filled for {event.token_id}")
            # Mocking position sync for demonstration purposes:
            # If strategy bought, increase. If sold, decrease.
            # This is typically handled by an upstream `PortfolioSyncService` on real agents.
        return []
