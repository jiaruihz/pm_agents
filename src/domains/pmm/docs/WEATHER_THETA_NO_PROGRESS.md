...
- 策略实现：
  - `src/domains/pmm/strategies/weather_theta_no_v1.py`
- 引擎接入（可运行）：
  - `src/domains/pmm/engine/tick_engine.py`
- 回测接入（可回放）：
  - `src/domains/pmm/backtest/replay_runner.py`
- 策略导出注册：
  - `src/domains/pmm/strategies/__init__.py`
- 文档接入（概要级）：
  - `src/domains/pmm/docs/STRATEGY_PLAYBOOK.md`
  - `src/domains/pmm/docs/ARCHITECTURE.md`
  - `src/domains/pmm/docs/BACKTEST_SCENARIO_METHOD.md`
- 单元测试：
