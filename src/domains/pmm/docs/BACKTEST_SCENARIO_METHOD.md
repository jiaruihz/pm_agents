...
> 文档分工：`src/domains/pmm/docs/archive/EXECUTION_PLAN_REALDATA_MULTI_LEVEL.md` 负责阶段计划，本文件负责可执行细则，避免冗余维护。

## 文件结构

```text
src/domains/pmm/backtest/
...
```
src/domains/pmm/backtest/.artifacts/results/<scenario_id>/
...
```
src/domains/pmm/backtest/.artifacts/results_all/
├── <scenario_id>/    各场景目录
...
```
src/domains/pmm/backtest/.artifacts/results_compare/
...
```
src/domains/pmm/backtest/.artifacts/results_compare_all_strategies/
├── <scenario_id>__<profile_name>/
│   ├── metrics.jsonl
│   ├── actions.jsonl
│   └── summary.json
...
```
src/domains/pmm/backtest/.artifacts/results_fill_models/
...
python scripts/python/pmm_backtest.py run \
  --scenario src/domains/pmm/backtest/scenarios/b50_oscillating_fill.json \
  --out-dir src/domains/pmm/backtest/.artifacts/results
...
python scripts/python/pmm_backtest.py run-all-fill-models \
  --scenarios-dir src/domains/pmm/backtest/scenarios \
  --out-dir src/domains/pmm/backtest/.artifacts/results_fill_models \
  --fill-models conservative,optimistic

# 同场景多参数对比（先用于单档，后续可直接纳入多档）
python scripts/python/pmm_backtest.py compare \
  --scenario src/domains/pmm/backtest/scenarios/b50_oscillating_fill.json \
  --profiles src/domains/pmm/backtest/compare_profiles_all_strategies.json \
  --out-dir src/domains/pmm/backtest/.artifacts/results_compare

# 全场景 × 全策略对比，并生成图表
python scripts/python/pmm_backtest.py compare-all \
  --scenarios-dir src/domains/pmm/backtest/scenarios \
  --profiles src/domains/pmm/backtest/compare_profiles_all_strategies.json \
  --out-dir src/domains/pmm/backtest/.artifacts/results_compare_all_strategies \
  --plot
...
python scripts/python/pmm_backtest.py plot \
  --result-dir src/domains/pmm/backtest/.artifacts/results/b50_oscillating_fill
...
python scripts/python/pmm_backtest.py record-live \
  ...
  --duration 300 \
  --interval 1.0 \
  --out-scenario src/domains/pmm/backtest/.artifacts/recorded/recorded_live.json \
  --out-jsonl src/domains/pmm/backtest/.artifacts/recorded/recorded_live.jsonl.gz \
  --initial-usdc 100 \
...
python scripts/python/pmm_backtest.py convert-live \
  --jsonl src/domains/pmm/backtest/.artifacts/recorded/recorded_live.jsonl.gz \
  --out-scenario src/domains/pmm/backtest/.artifacts/recorded/recorded_live_converted.json \
  --tokens <YES_TOKEN_ID>,<NO_TOKEN_ID> \
  --initial-usdc 100 \
  --initial-positions-json '{"<YES_TOKEN_ID>":50,"<NO_TOKEN_ID>":50}'
...
python scripts/python/pmm_backtest.py validate \
  --scenario src/domains/pmm/backtest/.artifacts/recorded/recorded_live_converted.json
...
- 回测和录制产物默认写入 `src/domains/pmm/backtest/.artifacts/`，不入 git。
- 长期保留仅保留 `src/domains/pmm/backtest/metadata/` 下汇总元数据文件。
...
校验器：`src/domains/pmm/backtest/scenario_validator.py`
