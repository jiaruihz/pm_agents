...
python scripts/python/pmm_orderbook_capture.py capture \
  ...
  --duration 180 \
  --interval 1.0 \
  --out-scenario src/domains/pmm/backtest/.artifacts/recorded/sample.json

# 把 jsonl 转成可直接 replay 的 scenario
python scripts/python/pmm_orderbook_capture.py convert \
  --jsonl src/domains/pmm/backtest/.artifacts/recorded/sample.jsonl.gz \
  --out-scenario src/domains/pmm/backtest/.artifacts/recorded/sample_converted.json
```
...
- 产物分层：
  - `src/domains/pmm/backtest/.artifacts/`：运行产物，默认不入库
  - `src/domains/pmm/backtest/metadata/`：汇总元数据，可入库
