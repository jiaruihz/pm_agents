...
description: 追踪 weather 的 signal candidate→signal→plan→order→fill→settlement 完整血缘，复盘单日、单城、单 market、单 order 或异常执行。用于为什么下这单、为什么没下/没成交、逐笔复盘、duplicate/overbuy/share-cap、exchange rejection。先读精确 raw，再用 fact_trades 补 fill/fee/PnL；禁止为单笔问题全量重建或重写 PnL。
---
...
1. 当前 Mac strategy runtime 的 `events/opportunities/plans/orders/fills` 原始记录。
2. authenticated exchange response / order state / transaction hash。
3. canonical `signals → plans → orders → fills`。
...
- signal 有、plan 无：selector/eligibility/duplicate gate。
