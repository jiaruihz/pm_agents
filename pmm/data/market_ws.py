...
class LocalOrderBookStore:
    ...
    def set_snapshot(self, token_id: str, bids: Any, asks: Any, event_ts: Optional[float] = None) -> None:
        book = self._ensure(token_id)
        book["bids"] = self._normalize_side(bids)
        book["asks"] = self._normalize_side(asks)
        self._mark_updated(token_id, event_ts)

    def apply_updates(self, token_id: str, side: str, levels: Any, event_ts: Optional[float] = None) -> None:
        book = self._ensure(token_id)
    ...
    def get_event_ts(self, token_id: str) -> float:
        return float(self._event_ts.get(token_id, 0.0))
    ...
    def _mark_updated(self, token_id: str, event_ts: Optional[float]) -> None:
        self._updated_at[token_id] = time.time()
        if event_ts and event_ts > 0:
            self._event_ts[token_id] = float(event_ts)
        elif token_id not in self._event_ts:
            self._event_ts[token_id] = self._updated_at[token_id]
...
class MarketWsFeed:
    ...
    def get_event_ts(self, token_id: str) -> float:
        return self.books.get_event_ts(token_id)
    ...
    def _handle_event(self, event: Dict[str, Any]) -> None:
        ...
        if bids is not None or asks is not None or event_type in {"book", "snapshot"}:
            self.books.set_snapshot(token_id, bids or [], asks or [], event_ts=event_ts)
            return
        ...
        for ch in changes:
            ...
            if side in {"buy", "bid", "bids"}:
                self.books.apply_updates(token_id, "buy", [ch], event_ts=event_ts)
            elif side in {"sell", "ask", "asks"}:
                self.books.apply_updates(token_id, "sell", [ch], event_ts=event_ts)

    def _extract_event_ts(self, event: Dict[str, Any]) -> float:
        candidates = [
            event.get("ts"),
            event.get("timestamp"),
            event.get("time"),
