from scripts.ops.weather_fast_source_city_policy import CITY_POLICIES, MAX_NO_ASK, configured_city_policies
...
def test_default_policy_separates_live_trials_from_shadow_cities():
    ...
    assert policies["Tokyo"].shares_per_trade == 15.0
    assert policies["Tokyo"].max_shares_per_market == 15.0
    assert policies["Busan"].shares_per_trade == 15.0
...
def test_all_fast_source_city_policies_share_the_live_ask_cap():
    assert MAX_NO_ASK == 0.97
    assert {policy.max_no_ask for policy in CITY_POLICIES.values()} == {MAX_NO_ASK}
