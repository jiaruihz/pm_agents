...
def test_rest_health_accepts_complete_hot_books_with_cold_failures(tmp_path) -> None:
    latest = tmp_path / "latest.json"
    latest.write_text(
        json.dumps(
            {
                "status": "ok_with_cold_book_failures",
                "available_at_utc": "2026-08-09T02:59:30Z",
                "batch_capture_id": "batch-cold-partial",
            }
        )
    )
    ...
    assert health["healthy"] is True
    assert health["status"] == "ok_with_cold_book_failures"
