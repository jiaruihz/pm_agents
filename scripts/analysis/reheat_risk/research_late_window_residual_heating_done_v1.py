#!/usr/bin/env python3
"""Per-poll PIT replay for exact-bracket residual capture using heating-done features.
...
Price is not an entry filter; it is only an execution/EV measurement.
"""
...
OUT_DIR = ROOT / "docs/analysis/2026-07/generated/late_window_residual_heating_done_v1"
OUT_MD = ROOT / "docs/analysis/2026-07/2026-07-07-late-window-residual-heating-done-v1.md"
...
def unit_step(unit: str) -> float:
    return 1.0 if str(unit).upper() == "F" else 0.5


def residual_done_features(row: dict[str, Any]) -> dict[str, Any]:
    """Bracket-specific PIT score for whether this exact residual leg is stale."""

    base = safe_float(row.get("heating_done_score_v1"))
    base = base if math.isfinite(base) else 0.0
    leg = str(row.get("leg") or "")
    unit = str(row.get("unit") or "")
    step = unit_step(unit)
    forecast_max = safe_float(row.get("forecast_max_native"))
    bracket_low = safe_float(row.get("target_bracket_low_native"))
    reasons = [str(row.get("heating_done_reasons_v1") or "")]
    score = base

    if leg in {"d2_no", "d3_no"} and math.isfinite(forecast_max) and math.isfinite(bracket_low):
        margin = bracket_low - forecast_max
        if margin >= 2.0 * step:
            score += 0.45
            reasons.append("forecast_below_far_no_by_2_steps")
        elif margin >= step:
            score += 0.35
            reasons.append("forecast_below_far_no_by_1_step")
        elif margin >= 0:
            score += 0.20
            reasons.append("forecast_below_far_no_bracket")
        elif margin < -step:
            score -= 0.25
            reasons.append("forecast_points_through_far_no")
    elif leg in {"current_yes", "d1_no"}:
        reasons.append("uses_day_heating_done")

    score = max(0.0, min(1.0, score))
    if score >= 0.70:
        bucket = "leg_residual_done_confirmed"
    elif score >= 0.55:
        bucket = "leg_residual_done_probable"
    elif score >= 0.40:
        bucket = "leg_residual_marginal"
    else:
        bucket = "leg_residual_not_done"
    return {
        "leg_residual_done_score_v1": round(score, 4),
        "leg_residual_done_bucket_v1": bucket,
        "leg_residual_done_reasons_v1": ";".join([reason for reason in reasons if reason])[:500],
    }
...
def book_candidates(df: pd.DataFrame) -> pd.DataFrame:
    return df[
        df["entry_price"].between(0.01, 0.999, inclusive="both")
        & (df["top_size"].fillna(0).ge(MIN_DEPTH_SHARES) | df["depth_5c"].fillna(0).ge(MIN_DEPTH_SHARES))
    ].copy()
...
def entry_policy_mask(df: pd.DataFrame, policy: str) -> pd.Series:
    ...
    heating_done = pd.to_numeric(df["heating_done_score_v1"], errors="coerce")
    leg_done = pd.to_numeric(df["leg_residual_done_score_v1"], errors="coerce")

    if policy == "all_first_cross":
        return true
    if policy == "current_yes_heating_done":
        return leg.eq("current_yes") & heating_done.ge(HEATING_DONE_THRESHOLD) & at_or_fading
    if policy == "d1_no_heating_done":
        return leg.eq("d1_no") & heating_done.ge(HEATING_DONE_THRESHOLD) & at_or_fading
    if policy == "d2_no_residual_done":
        return leg.eq("d2_no") & leg_done.ge(HIGHER_NO_RESIDUAL_THRESHOLD) & at_or_fading
    if policy == "d3_no_residual_done":
        return leg.eq("d3_no") & leg_done.ge(HIGHER_NO_RESIDUAL_THRESHOLD) & at_or_fading
    if policy == "heating_done_residual_combo":
        return (
            entry_policy_mask(df, "current_yes_heating_done")
            | entry_policy_mask(df, "d1_no_heating_done")
            | entry_policy_mask(df, "d2_no_residual_done")
            | entry_policy_mask(df, "d3_no_residual_done")
        )
...
def fail_cases(df: pd.DataFrame, limit: int = 80) -> pd.DataFrame:
    cols = [
        ...
        "heating_done_score_v1",
        "leg_residual_done_score_v1",
        "path_state",
        "forecast_gap_bucket",
        "question",
    ]
    bad = df[df["settled"].fillna(False).astype(bool) & ~df["win"].fillna(False).astype(bool)].copy()
    bad = bad.drop_duplicates(["city", "target_date", "snapshot_ts_utc", "leg", "bracket"], keep="first")
...
def write_report(payload: dict[str, Any], out_md: Path) -> None:
    ...
    lines = [
        ...
        f"- book-executable rows before first-cross, no price filter: {summary['book_rows']}",
        f"- legacy strict 1-5pt rows, diagnostics only: {summary['strict_rows']}",
        ...
        "",
        "## Heating-Done Slices",
        table(pd.DataFrame(summary["first_by_heating_done"]), ["leg", "heating_done_bucket_v1", "rows", "settled_rows", "active_dates", "hit_rate", "roi", "roi_ci_low", "roi_ci_high"]),
        "",
        "## Leg Residual-Done Slices",
        table(pd.DataFrame(summary["first_by_leg_done"]), ["leg", "leg_residual_done_bucket_v1", "rows", "settled_rows", "active_dates", "hit_rate", "roi", "roi_ci_low", "roi_ci_high"]),
        "",
        "## Basket",
        ...
        "",
        "## Failure Cases",
        table(pd.DataFrame(payload["failure_cases"]), ["city", "target_date", "ts_beijing", "leg", "bracket", "entry_price", "residual_points", "pnl_per_share", "running_value", "forecast_max_native", "forecast_peak_delta_hours_local", "heating_done_score_v1", "leg_residual_done_score_v1", "path_state", "forecast_gap_bucket"], max_rows=80),
        "",
        ...
        table(pd.DataFrame(payload["chengdu_case"]), ["ts_beijing", "leg", "bracket", "entry_price", "residual_points", "running_value", "forecast_max_native", "forecast_peak_delta_hours_local", "heating_done_score_v1", "leg_residual_done_score_v1", "path_state"], max_rows=30),
        "",
        "## Chengdu Policy Rows",
        table(pd.DataFrame(payload["chengdu_policy_rows"]), ["entry_policy", "ts_beijing", "leg", "bracket", "entry_price", "residual_points", "running_value", "forecast_max_native", "forecast_peak_delta_hours_local", "heating_done_score_v1", "leg_residual_done_score_v1", "path_state"], max_rows=40),
        "",
        "## Contract Gates",
...
def main() -> None:
    ...
    book_all = book_candidates(raw)
    strict_all = strict_candidates(raw)
    headline = book_all[book_all["target_date"].isin(complete_dates)].copy()
    first = first_cross(headline)
    policy_names = [
        "current_yes_heating_done",
        "d1_no_heating_done",
        "d2_no_residual_done",
        "d3_no_residual_done",
        "heating_done_residual_combo",
    ]
    ...
    failures = fail_cases(policy_rows if not policy_rows.empty else first)
    chengdu_case = book_all[(book_all["city"].eq("Chengdu")) & (book_all["target_date"].eq("2026-07-06"))].copy()
    ...
    book_all.to_csv(args.out_dir / "book_executable_rows.csv", index=False)
    ...
    incomplete_dates = sorted(set(book_all["target_date"]) - complete_dates)
    ...
    summary = {
        ...
        "first_by_heating_done": summarize(first, ["leg", "heating_done_bucket_v1"]),
        "first_by_leg_done": summarize(first, ["leg", "leg_residual_done_bucket_v1"]),
    ...
    policy_df = pd.DataFrame(summary["policy_by_name"])
    forward = (
        policy_df[policy_df.get("period", pd.Series(dtype=str)).astype(str).str.startswith("forward")]
        if not policy_df.empty
        else pd.DataFrame()
    )
    positive_forward = (
        forward[pd.to_numeric(forward.get("roi", pd.Series(dtype=float)), errors="coerce").gt(0)]
        if not forward.empty
        else pd.DataFrame()
    )
    verdict = (
        "Per-poll replay now separates physical residual state from price: entry is based on "
        "`heating_done_score_v1` and bracket-specific `leg_residual_done_score_v1`, while price "
        "is only reported as execution/EV. "
        "This fixes the overly rigid 17:00 and 1-5c entry gates, but the available settled forward "
        "window is still too small for live promotion. "
        "Conclusion: `inconclusive_research_shadow_only`; do not change live."
    )
    if not positive_forward.empty:
        verdict += (
            " Positive forward point estimates exist for feature policies: "
            f"{', '.join(sorted(positive_forward['entry_policy'].astype(str).unique()))}."
        )
