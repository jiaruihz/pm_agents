...
OUTPUT_DIR="${CURRENT_YES_CORE_CARRY_PRE_LIVE_OUTPUT_DIR:-$ROOT/runtime/weather_edge_v1/current_yes_core_carry_pre_live_v1}"
SESSION="${CURRENT_YES_CORE_CARRY_PRE_LIVE_TMUX_SESSION:-weather_current_yes_core_carry_pre_live_v1}"
TMUX_SOCKET="$(weather_jrs_tmux_start_socket)"
LOG_FILE="$OUTPUT_DIR/runner.log"

mkdir -p "$OUTPUT_DIR"
if pgrep -f "weather_current_yes_core_carry_pre_live_v1.py loop" >/dev/null 2>&1; then
  echo "current-YES core carry pre-live already running"
  exit 0
fi

weather_jrs_tmux "$TMUX_SOCKET" new-session -d -s "$SESSION" \
  "cd '$ROOT' && exec '$PY' -u scripts/ops/weather_current_yes_core_carry_pre_live_v1.py loop \
