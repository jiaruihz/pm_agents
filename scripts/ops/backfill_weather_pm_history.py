...
def event_slug(city_cfg: dict[str, Any], target_date: str) -> str:
    dt = datetime.strptime(target_date, "%Y-%m-%d")
    date_slug = f"{dt.strftime('%B')}-{dt.day}-{dt.year}".lower()
    return f"highest-temperature-in-{city_cfg['slug']}-on-{date_slug}"
...
def fetch_event(client: httpx.Client, city: str, target_date: str) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    slug = event_slug(FULL_CITY_CONFIGS[city], target_date)
    try:
        ...
        for target_date in iter_dates(args.start_date, args.end_date):
            for city in cities:
                ...
                if out.exists() and not args.overwrite:
                    stats["cached"] += 1
                    continue
                event, error = fetch_event(client, city, target_date)
                if event is None:
