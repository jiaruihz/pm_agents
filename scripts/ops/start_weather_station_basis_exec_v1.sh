...
TMUX_SESSION="${WEATHER_STATION_BASIS_EXEC_TMUX_SESSION:-weather_station_basis_exec_v1}"
...
weather_jrs_tmux "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" \
  "cd $(printf '%q' "$PROJECT_DIR") && exec scripts/ops/weather_station_basis_exec_v1_loop.sh > $(printf '%q' "$OUT_FILE") 2>&1"
echo "tmux:$TMUX_SESSION" >"$PID_FILE"
echo "started station-basis v1 exec loop (dry_run) tmux_socket=$TMUX_SOCKET session=$TMUX_SESSION log=$OUT_FILE"
