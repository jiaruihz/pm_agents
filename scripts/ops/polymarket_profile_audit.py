...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Audit a Polymarket profile from username, URL, or wallet.")
    ...
    parser.add_argument("--out-file", default="", help="summary json output path")
    parser.add_argument("--raw-dir", default="", help="raw payload directory")
    return parser
...
def print_summary(result: dict) -> None:
    ...
    raw_dir = Path(result["output_dir"]) / "raw"
    if raw_dir.exists():
...
def main() -> None:
    ...
    if args.out_file:
        Path(args.out_file).write_text(json.dumps(result["summary"], ensure_ascii=False, indent=2), encoding="utf-8")
