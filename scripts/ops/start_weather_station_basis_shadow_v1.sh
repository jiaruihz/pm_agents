...
TMUX_SESSION="${WEATHER_STATION_BASIS_SHADOW_TMUX_SESSION:-weather_station_basis_shadow_v1}"
LOG_DIR="$PROJECT_DIR/runtime/weather_edge_v1/station_basis_shadow_v1"
...
weather_jrs_tmux "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" \
  "cd $(printf '%q' "$PROJECT_DIR") && exec bash scripts/ops/weather_station_basis_shadow_v1_loop.sh > $(printf '%q' "$OUT_FILE") 2>&1"
echo "tmux:$TMUX_SESSION" >"$PID_FILE"
echo "started station-basis shadow v1 loop tmux_socket=$TMUX_SOCKET session=$TMUX_SESSION log=$OUT_FILE"
