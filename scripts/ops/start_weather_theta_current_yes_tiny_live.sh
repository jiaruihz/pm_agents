...
MIN_AVAILABLE_NOTIONAL="${MIN_AVAILABLE_NOTIONAL:-0}"
...
MAX_LOCAL_HOUR="${MAX_LOCAL_HOUR:-15}"
LLM_PREFLIGHT="${LLM_PREFLIGHT:-0}"
LLM_PREFLIGHT_BACKEND="${LLM_PREFLIGHT_BACKEND:-codex_cli}"
LLM_PREFLIGHT_MODE="${LLM_PREFLIGHT_MODE:-advisory}"
LLM_PREFLIGHT_PROMPT_VERSION="${LLM_PREFLIGHT_PROMPT_VERSION:-current_yes_codex_v2_veto_loss_detector}"
LLM_PREFLIGHT_MIN_BLOCK_CONFIDENCE="${LLM_PREFLIGHT_MIN_BLOCK_CONFIDENCE:-0.60}"
LLM_PREFLIGHT_TIMEOUT_SECONDS="${LLM_PREFLIGHT_TIMEOUT_SECONDS:-180}"
LLM_PREFLIGHT_CODEX_MODEL="${LLM_PREFLIGHT_CODEX_MODEL:-}"
LLM_PREFLIGHT_CODEX_REASONING_EFFORT="${LLM_PREFLIGHT_CODEX_REASONING_EFFORT:-low}"
...
PEAK_STATE_V2_MAX_D_TMPF_3H="${PEAK_STATE_V2_MAX_D_TMPF_3H:-2.0}"
...
if [[ "$LLM_PREFLIGHT" == "1" ]]; then
  args+=(
    ...
    --llm-preflight-backend "$LLM_PREFLIGHT_BACKEND"
    --llm-preflight-mode "$LLM_PREFLIGHT_MODE"
    --llm-preflight-prompt-version "$LLM_PREFLIGHT_PROMPT_VERSION"
    ...
    --llm-preflight-codex-model "$LLM_PREFLIGHT_CODEX_MODEL"
    --llm-preflight-codex-reasoning-effort "$LLM_PREFLIGHT_CODEX_REASONING_EFFORT"
  )
fi
...
if [[ "$ENABLE_PEAK_FORMING_LIVE" == "1" ]]; then
  args+=(
    ...
    --peak-state-v2-max-d-tmpf-3h "$PEAK_STATE_V2_MAX_D_TMPF_3H"
