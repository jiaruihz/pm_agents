...
TMUX_SESSION="${ALL_YES_UNDERROUND_MICRO_TMUX_SESSION:-all_yes_underround_micro_paper_v0}"
...
weather_jrs_tmux "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" \
  "cd $(printf '%q' "$PROJECT_DIR") && exec bash scripts/ops/all_yes_underround_micro_paper_loop_v0.sh > $(printf '%q' "$OUT_FILE") 2>&1"
echo "tmux:$TMUX_SESSION" > "$PID_FILE"
echo "started all-YES underround micro paper loop tmux_socket=$TMUX_SOCKET session=$TMUX_SESSION log=$OUT_FILE"
