...
tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" \
  "cd '$PROJECT_DIR' && export WEATHER_DATA_FEED_RUNTIME_ROOT='$RUNTIME_ROOT' && export MAC_WEATHER_DATA_FEED_LOOP_CHILD=1 && exec '$PROJECT_DIR/scripts/ops/start_mac_weather_data_feed_loop.sh' >> '$LOG_FILE' 2>&1"
