...
TASK_BUCKETS = ("queue", "active", "archive")
PROJECT_DIRS = (*TASK_BUCKETS, "handoffs", "packets")
REQUIRED_FIELDS = {
    "TASK_ID",
    "PROJECT_ID",
    "WORKSTREAM",
...
def resolve_task(value: str) -> Path:
    ...
    if (
        path.parent.name not in TASK_BUCKETS
        or path.parent.parent.parent != TASK_ROOT.resolve()
        or not project_path(path.parent.parent.name).is_file()
    ):
        raise SystemExit(
            f"task must be directly under tasks/<PROJECT_ID>/queue|active|archive: {path}"
        )
    if not path.is_file():
...
def project_area(project_id: str, name: str) -> Path:
    return TASK_ROOT / project_id / name


def load_task(path: Path) -> dict[str, str]:
    task = parse_fields(path.read_text(encoding="utf-8"))
    project_id = task.get("PROJECT_ID", "")
    path_project_id = path.parent.parent.name
    if project_id != path_project_id:
        raise SystemExit(
            f"task PROJECT_ID {project_id or '<missing>'} does not match directory {path_project_id}"
        )
    return task


def registered_project_ids() -> list[str]:
    return sorted(path.stem for path in PROJECT_ROOT.glob("*.md"))


def project_task_roots() -> list[Path]:
    reserved = {PROJECT_ROOT.name, "templates"}
    return sorted(
        path for path in TASK_ROOT.iterdir() if path.is_dir() and path.name not in reserved
    )
...
def task_files() -> list[Path]:
    return sorted(
        path
        for project_root in project_task_roots()
        for bucket in TASK_BUCKETS
        for path in (project_root / bucket).glob("*.md")
    )
...
def validate() -> list[str]:
    ...
    for path in sorted(PROJECT_ROOT.glob("*.md")):
        ...
        if path.stem != project_id:
            errors.append(f"{path.relative_to(ROOT)}: filename must match PROJECT_ID")
        for name in PROJECT_DIRS:
            directory = project_area(project_id, name)
            if not directory.is_dir():
                errors.append(f"{path.relative_to(ROOT)}: missing task directory {directory.relative_to(ROOT)}")
    ...
    for directory in project_task_roots():
        if directory.name not in project_ids:
            errors.append(f"unregistered project task directory: {directory.relative_to(ROOT)}")
    ...
    for path in task_files():
        ...
        path_project_id = path.parent.parent.name
        if data["PROJECT_ID"] != path_project_id:
            errors.append(
                f"{path.relative_to(ROOT)}: PROJECT_ID must match task directory {path_project_id}"
            )
        status = data["STATUS"]
        ...
        handoff_root = project_area(data["PROJECT_ID"], "handoffs")
        try:
            require_within(handoff, handoff_root, "handoff")
        except SystemExit as error:
            errors.append(f"{path.relative_to(ROOT)}: {error}")
        else:
            if handoff.parent != handoff_root.resolve() or handoff.name != f"{task_id}.md":
                errors.append(
                    f"{path.relative_to(ROOT)}: HANDOFF_PATH must be "
                    f"{project_area(data['PROJECT_ID'], 'handoffs') / f'{task_id}.md'}"
                )
        ...
        if status == "ACTIVE":
            ...
            if path.parent.name != "active":
                errors.append(f"{path.relative_to(ROOT)}: ACTIVE task must be in project active directory")
        if status == "DRAFT" and path.parent.name != "queue":
            errors.append(f"{path.relative_to(ROOT)}: DRAFT task must be in project queue directory")
        if status == "ACCEPTED" and path.parent.name != "archive":
            errors.append(f"{path.relative_to(ROOT)}: ACCEPTED task must be in project archive directory")
...
def command_new(args: argparse.Namespace) -> int:
    ...
    if not re.fullmatch(r"[A-Z][A-Z0-9]*(?:-[A-Z0-9]+)*", workstream):
        raise SystemExit("workstream must use uppercase A-Z/0-9 segments separated by hyphens")
    ...
    destination = project_area(project_id, "queue") / f"{task_id}.md"
...
def command_handoff(args: argparse.Namespace) -> int:
    ...
    require_within(destination, project_area(task["PROJECT_ID"], "handoffs"), "handoff")
...
def command_pack(args: argparse.Namespace) -> int:
    ...
    if handoff is not None:
        require_within(handoff, project_area(task["PROJECT_ID"], "handoffs"), "handoff")
    ...
    packet_root = project_area(task["PROJECT_ID"], "packets")
    destination = resolve_from_root(args.output) if args.output else packet_root / f"{run_id}.md"
    require_within(destination, packet_root, "packet output")
