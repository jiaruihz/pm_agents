...
OUTPUT_ROOT="${WEATHER_DATA_FEED_FORECAST_OUTPUT_ROOT:-$(weather_production_path "$PROJECT_DIR" forecast_output_root)}"
STRATEGY_SNAPSHOT_DIR="${WEATHER_STRATEGY_PAPER_SNAPSHOT_DIR:-$(weather_production_path "$PROJECT_DIR" strategy_paper_snapshot_dir)}"
...
loop_command="source '$PROJECT_DIR/scripts/ops/weather_process_supervision.sh'; if [[ -f '$PROJECT_DIR/.env' ]]; then set -a; source '$PROJECT_DIR/.env'; set +a; fi; export WEATHER_DATA_FEED_FORECAST_PROXY=\"\${WEATHER_DATA_FEED_FORECAST_PROXY:-\${WEATHER_DATA_FEED_MARKET_PROXY:-}}\"; export WEATHER_DATA_FEED_WEATHER_PROXY=\"\${WEATHER_DATA_FEED_WEATHER_PROXY:-\${WEATHER_DATA_FEED_FORECAST_PROXY:-}}\"; while true; do date -u +\"[forecast_curve_collector] start_utc=%Y-%m-%dT%H:%M:%SZ\"; WEATHER_DATA_FEED_OUTPUT_ROOT='$OUTPUT_ROOT' WEATHER_DATA_FEED_CACHE_ROOT='$CACHE_ROOT' WEATHER_DATA_FEED_FORECAST_LIVE_REFRESH_SEC='$REFRESH_SEC' weather_run_with_timeout '$TIMEOUT_SEC' '$PY' -u -m weather_data_feed_service.forecast_curve_collector --output-root '$OUTPUT_ROOT' --status-path '$STATUS_PATH' || true; weather_run_with_timeout '$TIMEOUT_SEC' '$PY' -u -m weather_data_feed_service forecast-enrichment -- --output-dir '$ENRICHMENT_OUTPUT' --market-snapshot-dir '$STRATEGY_SNAPSHOT_DIR' --include-station-diff --include-research-cities --open-meteo-refresh-sec '$ENRICHMENT_REFRESH_SEC' --no-single-runs --max-workers 4 || true; date -u +\"[forecast_curve_collector] done_utc=%Y-%m-%dT%H:%M:%SZ\"; sleep '$INTERVAL_SEC'; done"
