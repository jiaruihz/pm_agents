...
PY="${WEATHER_KNMI_LADDER_PYTHON:-$PROJECT_DIR/.venv/bin/python}"
SOURCE="${WEATHER_KNMI_LADDER_SOURCE:-$RUNTIME_ROOT/output/knmi_open_data/knmi_observations.jsonl}"
PAPER="${WEATHER_KNMI_LADDER_PAPER_SNAPSHOTS:-$RUNTIME_ROOT/targeted_output/paper_snapshots}"
ORDERBOOKS="${WEATHER_KNMI_LADDER_ORDERBOOK_SNAPSHOTS:-$RUNTIME_ROOT/full_ladder_output/orderbook_snapshots}"
...
cmd=(
  "$PY" -u "$PROJECT_DIR/scripts/ops/weather_knmi_first_seen_ladder_shadow_v1.py"
...
weather_jrs_tmux "$SOCKET" new-session -d -s "$SESSION" \
  "cd '$PROJECT_DIR' && export PYTHONPATH='$PROJECT_DIR' && exec $(printf '%q ' "${cmd[@]}") >> '$OUTPUT/forward.log' 2>&1"
