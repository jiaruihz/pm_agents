...
screen -dmS "$SESSION" sh -c \
  "cd $(printf '%q' "$PROJECT_DIR") && exec $(printf '%q' "$PROJECT_DIR/.venv/bin/python") $(printf '%q' "$PROJECT_DIR/scripts/ops/weather_live_runtime_patrol.py") --loop --interval-sec 60 --stop-runner-on-submit-failure >> $(printf '%q' "$LOG_FILE") 2>&1"
