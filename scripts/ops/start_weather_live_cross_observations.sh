...
weather_jrs_tmux "$TMUX_SOCKET" new-session -d -s "$SESSION" \
  "cd '$ROOT' && set -a; [[ -f '$ENV_FILE' ]] && source '$ENV_FILE' || true; set +a; exec '$PY' -u '$ROOT/scripts/ops/weather_live_cross_observations_loop.py' --output-dir '$OUTPUT_DIR' --notify-path '$NOTIFY_PATH' --base-loop-interval-sec 20 --max-workers 8 --timeout-sec 5 --sources jma_amedas singapore_mss fmi amos_runway mgm ims_lod noaa_madis_hfmetar --cities Helsinki Busan Singapore Tokyo Seoul TelAviv Ankara Istanbul Atlanta Miami SanFrancisco --source-min-interval-sec jma_amedas=300 --source-min-interval-sec singapore_mss=20 --source-min-interval-sec fmi=60 --source-min-interval-sec amos_runway=20 --source-min-interval-sec mgm=300 --source-min-interval-sec ims_lod=300 --source-min-interval-sec noaa_madis_hfmetar=300 --source-window-min-interval-sec jma_amedas=5-8,15-18,25-28,35-38,45-48,55-58:2 --source-window-min-interval-sec fmi=1-6,11-16,21-26,31-36,41-46,51-56:2 --fast-lane amos_core=amos_runway:Busan,Seoul@5 >> '$LOG_FILE' 2>&1"
