...
INTERVAL_SEC="${KNMI_OPEN_DATA_INTERVAL_SEC:-300}"
...
weather_jrs_tmux "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" \
  "cd '$SERVICE_DIR' && set -a && source '$ENV_FILE' && set +a && exec '$SERVICE_DIR/.venv/bin/python' -u -m weather_data_feed_service.knmi_open_data --output-dir '$OUTPUT_DIR' --interval-sec '$INTERVAL_SEC' --timeout-sec '$TIMEOUT_SEC' >> '$LOG_FILE' 2>&1"
