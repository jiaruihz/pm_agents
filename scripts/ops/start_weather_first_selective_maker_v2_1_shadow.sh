...
SUBSCRIPTION_EPOCHS="${WEATHER_FIRST_SELECTIVE_MAKER_V2_1_SUBSCRIPTION_EPOCHS:-/Volumes/jrs/weather_data_feed_service_runtime/market_books/ws_incremental/subscription_epochs}"
EXECUTION_EVIDENCE="${WEATHER_FIRST_SELECTIVE_MAKER_V2_1_EXECUTION_EVIDENCE:-/Volumes/jrs/weather_data_feed_service_runtime/market_books/ws_incremental/execution_evidence_v1/public_books}"
...
CMD="cd '$ROOT' && export PYTHONPATH='$ROOT' && exec '$PY' -u '$ROOT/scripts/ops/weather_first_selective_maker_v2_1_shadow.py' --actions '$ACTIONS' --packets '$PACKETS' --market-books '$BOOKS' --subscription-epochs-root '$SUBSCRIPTION_EPOCHS' --execution-evidence-root '$EXECUTION_EVIDENCE' --output-dir '$OUTPUT_DIR' --loop-seconds 15 --maximum-book-age-seconds 5 --taker-fee-rate 0.05 --taker-fee-exponent 1 --capture-ttl-minutes 10 --capture-checkpoint-grace-seconds 20 --max-daily-capture-demands 32 --max-active-capture-tokens 16 >> '$OUTPUT_DIR/runner.log' 2>&1"
