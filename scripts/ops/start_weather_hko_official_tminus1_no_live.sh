...
TMUX_SOCKET="$(weather_jrs_tmux_start_socket "${WEATHER_HKO_LOCK_TMUX_SOCKET:-}")"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  [[ "$old" == screen:* ]] && screen -S "${old#screen:}" -X quit 2>/dev/null || true
  [[ "$old" == tmux:* ]] && tmux -L "$TMUX_SOCKET" kill-session -t "${old#tmux:}" 2>/dev/null || true
fi
tmux -L "$TMUX_SOCKET" kill-session -t "$TMUX_SESSION" 2>/dev/null || true
pkill -f "$PROJECT_DIR/scripts/ops/weather_hko_official_tminus1_no_live.py --loop" 2>/dev/null || true
printf -v quoted_cmd '%q ' "${cmd[@]}"
tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" "cd $(printf '%q' "$PROJECT_DIR") && set -a && { [ ! -f .env ] || . ./.env || true; } && set +a && exec $quoted_cmd >> $(printf '%q' "$LOG_FILE") 2>&1"
