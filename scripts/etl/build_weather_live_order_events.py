...
COLUMNS = [
    ...
    "tmax_probability_model_spec",
    "tmax_blend_p_current",
    "tmax_blend_p_d1",
    "tmax_blend_p_d2",
    "tmax_blend_p_tail",
    "final_yes",
...
def normalize_row(
    *,
    ...
    event = {
        ...
        "clob_client": safe_str(row.get("clob_client") or nested(row, "exchange_response", "clob_client")),
        "market_slug": safe_str(row.get("market_slug")),
        "market_event_id": safe_str(row.get("event_id")),
        ...
        "quote_mode": safe_str(row.get("quote_mode") or nested(row, "exchange_response", "quote_mode")),
        "quote_tick_size": safe_float(row.get("quote_tick_size") or nested(row, "exchange_response", "quote_tick_size")),
        "quote_reason": safe_str(row.get("quote_reason") or nested(row, "exchange_response", "quote_reason")),
        "risk_status": safe_str(row.get("risk_status")),
        "risk_reason": safe_str(row.get("risk_reason")),
        ...
        "shadow_decision": safe_str(row.get("shadow_decision")),
        "shadow_reason": safe_str(row.get("shadow_reason")),
        "entry_price_window": safe_str(row.get("entry_price_window")),
        ...
        "notional_fraction": safe_float(row.get("notional_fraction")),
        "size_multiplier": safe_float(row.get("size_multiplier")),
        "taker_fraction": safe_float(row.get("taker_fraction")),
        "model_p_yes": safe_float(first_value(row, "model_p_yes", "model_p_yes_used", "model_token_probability")),
        "model_p_yes_raw": safe_float(row.get("model_p_yes_raw")),
        "model_p_yes_used": safe_float(row.get("model_p_yes_used") or row.get("model_token_probability")),
        "market_implied_p_yes": safe_float(row.get("market_implied_p_yes")),
        "edge": safe_float(first_value(row, "edge", "edge_used_yes", "quote_edge")),
        "quote_edge": safe_float(row.get("quote_edge") or nested(row, "exchange_response", "quote_edge")),
        "fee_adjusted_edge": safe_float(first_value(row, "fee_adjusted_edge", "required_quote_edge")),
        "estimated_taker_fee_usd": safe_float(row.get("estimated_taker_fee_usd")),
        "estimated_maker_rebate_usd": safe_float(row.get("estimated_maker_rebate_usd")),
        ...
        "source_plan_status": safe_str(row.get("source_plan_status")),
        "source_snapshot_path": safe_str(row.get("source_snapshot_path")),
        "tmax_probability_model_spec": safe_str(row.get("tmax_probability_model_spec")),
        "tmax_blend_p_current": safe_float(row.get("tmax_blend_p_current")),
        "tmax_blend_p_d1": safe_float(row.get("tmax_blend_p_d1")),
        "tmax_blend_p_d2": safe_float(row.get("tmax_blend_p_d2")),
        "tmax_blend_p_tail": safe_float(row.get("tmax_blend_p_tail")),
        "final_yes": final_yes,
