...
- 归档派生产物只可在明确删除授权下用 `weather_research_artifact_ctl.py prune-reproduced` 清理：必须 clean replay SHA 精确一致、无下游消费者，并把 producer/code/input/runtime inventory 写入 tombstone；raw、canonical 与仅语义近似的结果不得借此删除。
