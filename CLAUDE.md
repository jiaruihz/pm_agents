...
- **代理入口与节点切换也只有一个 owner**：所有生产 consumer 只使用 `production.yaml` 的固定本机 endpoint，不各自选择 Clash 节点、不自行重启。节点健康、双重失败确认、候选切换、单飞锁和审计只由 control-plane release 中的 `weather_market_proxy_ctl.py maintain-node` 管理；`weather_runtime_monitor` 每分钟请求一次自动恢复，同一 endpoint 下切节点不重启 consumer。Clash external controller 只可监听 loopback，secret 从环境/Keychain 读取，禁止恢复历史独立 failover LaunchAgent 或让多个进程并发改节点。自动恢复失败必须保留原节点并显式降级，不能静默改走 direct。
