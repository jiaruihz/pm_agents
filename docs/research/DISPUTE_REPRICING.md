...
状态：`dispute-only adjudication-divergence architecture / managed Mac shadow running / Codex court enabled / zero-notional only`。
...
forward queue 为 0；这轮只验证了无 update 时不会误触发。collector 与 court 已作为独立受管 runtime
在 Mac canonical JRS context 常驻；collector 30 秒轮询、court 60 秒轮询，exact model hash 与
zero-notional health 由 controller 检查。当前可以称为持续监听，但没有 post-deploy update signal，
因此不能据此声称裁决精度或交易 edge 已获 forward 验证。
...
两个入口都只生成 zero-notional paper records，不会提交 CLOB order。生产已启用 Codex CLI
`gpt-5.4 / medium` packet-only court；只有出现新 creator update packet 才产生实际 LLM 调用。
...
| `official_clarification_court_taker_25share_shadow_v1` | creator update 是否闭合 binding predicate，并通过 fundamental-intent 审查 | packet-only Outcome0/1 court；update→quote ≤300s；fresh 25-share ask；0.95 payout floor 后 edge ≥5¢；update cluster cap | 历史 development non-P4 30/30；Mac 受管 shadow 正在监听，当前 90 tracked、0 post-deploy update signal |
...
- **M3：受管 zero-notional service（已部署）**。dispute listener、adaptive
  bulletin watcher、capture-demand publisher、bounded court worker、统一 health 和 JRS tmux start contract
  已完成；两个 managed runtimes 分别加载 release `95fe0532`，共享 market-books owner 加载
  `269c1072`。controller health 与 post-change manifest 均 healthy，仍没有 live authority。
...
- 唯一 `weather_market_books` WS owner 的 cursor 已兼容多个 append-only demand stream，并验证
  ...
  dispute direct token 可进入同一个 subscription set，不需要 rule-lawyer 启动第二个 WS writer。生产
  owner 已加载 shared demand path。同一 token 的重叠 dispute/update demand 使用一对多
...
2026-08-13 首次生产启动同时修复了两个只会在真实部署暴露的 bootstrap 问题。第一轮把 90 个历史
未结算 dispute 误当 realtime first-seen，发布 180 个 P0 demand；12-token owner budget 正确拒绝了
大部分，未形成订单或资金动作。第二轮旋转目录时，旧 tmux pane 的 Python 尚未完全退出，向新目录
写入 84-case state 与 254 个 orphan checkpoint receipts。最终修复为：初始历史 case 只建档，只有
dispute age≤300 秒的真正新事件才保留 realtime trigger；controller stop/restart 同时等待已观察 pane
PID 消失，超时则 fail closed。两次原始尝试分别保留在
`bootstrap_attempt_20260813T0133Z` 与 `bootstrap_attempt_20260813T0137Z_orphan_race`，没有删除或混入
当前 canonical。干净重启后 90 cases / 180 books / 360 candidates 全部物化，capture demand、receipt、
intent、paper order/fill 均为 0，canonical errors=0。
...
- raw journal 现在是 `events/snapshots → signals + clarification_signals → trade_intents →
  paper_plans/orders/fills → shadow_positions/markouts`，并增量进入 `dispute.db`。Mac runtime 正在监听；
  初始 90 个历史 case 分批建档完成，未伪造实时 capture demand。
...
- collector 与 ledger 均为 read-only / zero-notional；已接无 venue client 的公共 paper execution handoff，
  actual shares/cost/notional 强制为 0。当前 paper order/fill=0，现金流=0。
