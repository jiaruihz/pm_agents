...
- `src/domains/research/cli.py`
  - CLI 入口，typer 命令集合，统一日志、命令参数与执行入口。
- `src/domains/research/pipeline.py`
...
- `src/domains/research/db.py`
  - SQLite 连接与 upsert 执行。
- `src/domains/research/schema.py`
  - 表结构与 UPSERT SQL 模板。
- `src/domains/research/storage.py`
  - 持久化与查询层（upsert + select）。
- `src/domains/research/parser.py`
  - LLM 解析：规则抽取 schema + 解析重试 + 落库。
- `src/domains/research/clients/llm.py`
  - OpenAI 兼容 LLM 调用封装。
- `src/domains/research/llm_prompts.py`
  - LLM 提示词（系统+few-shot+用户模板）。
- `src/domains/research/metrics.py`
  - 订单簿/价差指标计算。
- `src/domains/research/scoring.py`
  - 评分权重与组合逻辑。
- `src/domains/research/nodes/`
...
`src/domains/research/config.py` 从环境变量或 `.env` 读取：
...
`src/domains/research/schema.py` 定义表：
...
`src/domains/research/pipeline.py` 中 `sync_gamma()`：
...
`src/domains/research/pipeline.py` 的 `enrich_prices_orderbooks()`：
...
`src/domains/research/clients/http.py` 关键机制：
...
- **核心入口**：`src/domains/research/cli.py` → `pipeline.py`
