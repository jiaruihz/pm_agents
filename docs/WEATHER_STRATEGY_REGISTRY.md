...
| d1_weather_only_probability_challenger | D-1 固定 checkpoint 上，以多模型 consensus 定位最终 Tmax，并用部分分层 city/source bias、pooled empirical residual 与稳健尾部输出 settlement-native exact-bracket 全分布；first_seen 只作 PIT 时钟 | **`legacy challenger frozen / revision-repricing research started / clean exact-run forward pending / no-live-change`** | 5561 条 long-history + 18-date reconstructed development 选择 `87.5% ensemble mean + 12.5% assigned + full shrunk bias + 1.25× residual scale + 2% climatology tail`；已查看的 9-date secondary holdout logloss `1.9763→1.8506` 但仍输 market `1.5497`。legacy market offset 选择 `beta=.05`，M2/M3=`1.5546/1.5540` 均未优于 M0。revision×repricing 首轮审计发现旧 collector state 产生 2,380 个 backward lineage rows；原始 run/value/hash 未污染，派生 revision 字段排除。本地已修 chronological run history、assigned model、material-batch 去重与 batch availability，首轮 408 material batches/68 D-1 transitions 中 0 个属于新 complete forward，故只继续 collector/研究、不跑 ROI、不改 live；production collector 修复重载待显式确认 | [1] forecast quality · [robust-tail report](analysis/2026-08/2026-08-05-d1-weather-only-robust-tail-v1.md) · [revision/repricing audit](analysis/2026-08/2026-08-05-d1-forecast-revision-market-repricing-plan-v1.md) · [freeze spec](analysis/2026-08/2026-08-05-d1-weather-only-clean-forward-freeze-v1.json) |
