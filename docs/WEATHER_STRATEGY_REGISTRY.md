...
Updated: 2026-07-07 late-window residual split heads
...
| metar_cross_prev_no / fast_source_prev_no_trial_v1 | 快源先看到跨档后买 previous exact bracket NO；alpha 假设是 source→settlement-facing observation 的时间差 | **`collector-only / rejected-as-main-strategy`；当前旧进程应暂停** | 两个月盘口不等于两个月 profile-matched 快源 PIT；后者从 7/08 UTC 才持续保存。旧 58 rows / 5 dates 混入 7/13 condition fallback；canonical-only 只有 4 个可结算交易日，不能回答近 60 天 alpha，也不推荐 5-share。14 fills 中 12 笔因 FOK BUY=USDC spend 超 share cap，本地已改 post-only GTD 并默认持久 pause，但旧 PID 尚未重启 | [0]-[4] latency_arb · [denominator audit](analysis/2026-07/2026-07-14-source-event-denominator-audit-v2.md) · [share-cap incident](analysis/2026-07/2026-07-14-fast-source-share-cap-incident-v1.md) · [strategy reset](analysis/2026-07/2026-07-14-strategy-search-reset-v1.md) |
| hko_official_tminus1_no_live_v1 | 香港天文台 HKO 官方站已出现 floor(T) 后，只买最高温 `T-1 NO`；不消费 VHHH/METAR，不买 current YES | **`local-ready / not running`** | HKO Daily Extract 是 HK 结算源，floor 后 `T-1 NO` 是结构锁定；当前无 HKO live process / order，已发生 share-cap 影响为 0。同根 FOK hard-share 风险已在本地改为 post-only GTD，未部署/启动；后续仍只作 tiny forward probe，不作为 confirmed alpha | [0]-[2] latency_arb · [HK readiness](analysis/2026-07/2026-07-11-tokyo-hk-fast-source-live-readiness-v1.md) · [share-cap incident](analysis/2026-07/2026-07-14-fast-source-share-cap-incident-v1.md) |
...
| broad_price_reversal_v1 | 同一 exact bracket 连续小时 YES midpoint 移动至少 2c 后反向买入，不加 city/source/weather/ask filter | `research` / **rejected-as-main-strategy** | 宽分母：BUY NO 3,043 rows / 49 dates，fee ROI -16.1% CI[-23.4%,-8.8%]；BUY YES 649 rows / 48 dates，ROI -9.6% CI[-17.5%,-1.6%]。简单“市场反应过头”不存在，16-row current-YES 正切片不升格 | [1]-[2] market_structure_edge · [broad replay](analysis/2026-07/2026-07-14-broad-price-reversal-v1.md) |
| regime_routed_no_route_price_disciplined_tiny_live_v1 | `route_price_disciplined + no_pullback + row_risk_soft`：live-executable legs 只保留 fresh runway current-NO 与 capped d2-NO；false-fade reheat / cheap stale-tail current-NO 已改为 shadow-only diagnostics，不并入 pullback YES | **`live`（tiny-live forward probe）** | 替换旧 `regime_routed_no_soft_balanced_tiny_live_v1`。2026-06-29 起按 route-specific ask cap、row-risk soft sizing、market event date lineage、current-local-day filter、feature parity、duplicate city/date/token veto 执行；2026-07-04 guard fix 对 `false_fade_reheat_current_no`、`cheap_stale_tail_current_no` 加 `shadow_only_route_leg` 执行硬 veto，并把 runner 依赖收口到 stable adapter。Single Runs PIT backfill rejoin 后，离线 replay 主口径 `routed_capped_d2_no_relaxed70_first_eligible + soft_balanced` 变为 ROI -6.3% CI [-22.4%,+9.3%]；该 live 只能作为用户批准的微仓前向取证，不是 confirmed edge。 | [1]-[4] reheat_risk |
