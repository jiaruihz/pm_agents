...
| regime_routed_no_soft_balanced_tiny_live_v1 | `day_open/marginal_runway` 买 current-bracket NO；`day_forecast_capped` 买 d2 NO | **`paused` / incident** | 2026-06-25 曾作为 tiny-live probe 成交 NYC `82-83 NO` 两笔，随后暂停。根因：live runner 没接上回测 atlas 的 trend/cloud/wind/running-max freshness 特征，且 signal_id 随 snapshot 变化导致同 city/date/token 可重复下单；该 live 版本证据作废。只能保留 shadow/replay，恢复 live 前必须先通过 feature-parity replay、duplicate city/date/token gate、真实 target-date daily cap、deploy review。 | [1]-[4] reheat_risk |
