...
| low_price_yes_lottery_tiny_live_v1 | refined low-price BUY_YES longshot：`edge>=0.20`、ask `0.05..0.20`、`dist>0`、每 city-date 一笔，fresh-book maker-first；2026-07-06 起 live sizing=`score_tier_0p8_1p2_1p5_shares`（先按原 price tier 5-8c→6 shares、8-14c→8、14-20c→10，再用 frozen score tier 低/中/高=0.8x/1.2x/1.5x，最低 5 shares、最高 15 shares），并记录 fixed-cash / fixed-8-share / price-tier / score-tier shadow sizing、station-basis / p_cal / bracket-distance / book-state telemetry | **`live`（tiny-live forward probe）** | 用户 2026-07-02 批准微仓前向取证；2026-07-04 代码准备 price-tier sizing，但 canonical fill 显示 7/04 四张票仍是 fixed `$0.8/order`，实际 price-tier 从 7/05 Shanghai/Wellington 起生效；2026-07-06 用户批准按 score 轻微放大持仓，score-tier 历史回放 ROI +49.5% vs price-tier +41.8%，成本约 +28.9%，但仍只是 tiny-live sizing overlay，不是 confirmed alpha 或 size-up。2026-07-02..ECMWF 修复时刻的 ECMWF-assigned 城市行受 GFS fallback 污染，forward gate 需剔除或单独分层；核心风险仍是少数尾部命中驱动与成交质量；只按独立 strategy family 记录，不接 regime-routed NO runner | [1]-[4] pre_predict / forecast_quality · [selector refinement](analysis/2026-07/2026-07-02-low-price-yes-lottery-selector-refinement-v1.md) · [HeadA refinement](analysis/2026-07/2026-07-04-low-price-yes-heada-refinement-v1.md) · [score×dist sizing](analysis/2026-07/2026-07-06-low-price-yes-score-dist-sizing-v1.md) |
...
| hko_official_tminus1_no_live_v1 | 香港天文台 HKO 官方站已出现 floor(T) 后，只买最高温 `T-1 NO`；不消费 VHHH/METAR，不买 current YES | **`live`（tiny probe，5 shares/单/market）** | HKO Daily Extract 是 HK 结算源，floor 后 `T-1 NO` 是结构锁定；10 分钟 cadence 只影响机会率。2026-07-11 起 Mac 专用 FOK runner，fresh ask `<=0.93`、source detect 新鲜且上游 observation lag `<=30m`。前向执行取证，不作为已确认 alpha 或扩仓依据 | [0]-[2] latency_arb · [HK readiness](analysis/2026-07/2026-07-11-tokyo-hk-fast-source-live-readiness-v1.md) |
