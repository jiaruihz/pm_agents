...
| regime_routed_no_route_price_disciplined_tiny_live_v1 | `route_price_disciplined + no_pullback + row_risk_soft`：fresh runway / false-fade reheat / cheap stale-tail current-NO，以及 capped d2-NO；不并入 pullback YES | **`live`（tiny-live forward probe）** | 替换旧 `regime_routed_no_soft_balanced_tiny_live_v1`。2026-06-29 起按 route-specific ask cap、row-risk soft sizing、market event date lineage、current-local-day filter、feature parity、duplicate city/date/token veto 执行；建议首阶段 `base_notional=$5`、`daily_gross_cap=$5`、`max_orders=1`。研究结论仍是 `inconclusive_shadow_only`，所以该 live 只能作为用户批准的微仓前向取证，不是 confirmed edge。 | [1]-[4] reheat_risk |
