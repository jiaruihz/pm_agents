...
| regime_routed_no_soft_balanced_tiny_live_v1 | `day_open/marginal_runway` 买 current-bracket NO；`day_forecast_capped` 买 d2 NO | **`live`（tiny-live forward probe）** | 2026-06-25 曾作为 tiny-live probe 成交 NYC `82-83 NO` 两笔后暂停；根因是 live runner 与回测 atlas 特征不等价且同 city/date/token 可重复下单。2026-06-26 已恢复为微仓 forward probe：默认消费 `weather_data_feed` observation cache，feature parity 必须通过，duplicate city/date/token 会 veto，daily cap 按真实 target_date 计，snapshot/cache freshness 进 preflight。仍按前向取证处理，不是 confirmed edge。 | [1]-[4] reheat_risk |
