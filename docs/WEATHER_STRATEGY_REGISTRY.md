...
- HeadA/low-price YES 保留固定小份额 probe，旧 probability/score sizing 不恢复。
...
| low_price_yes_lottery_tiny_live_v1 | refined low-price BUY_YES longshot：`edge>=0.20`、ask `0.05..0.20`、`dist>0`、每 city-date 一笔，fresh-book maker-first；2026-07-12 起 live sizing 恢复为固定 `5 shares`，旧 fixed-cash / price-tier / score-tier 继续作为 shadow telemetry；station-basis / p_cal / bracket-distance / book-state 同步记录 | **`live`（tiny-live forward probe，未确认）** | 2026-07-02..07-13 actual 67 fills，已结算 cash+fee $32.975249、PnL -$1.175249、ROI -3.564%，另有 open cash $0.74；早期正收益未延续，因此 P0 仍只是 forward probe。2026-07-06..07-12 曾按 score tier 放大，但 live audit 显示 `model_p_yes` 严重高估且 forward 排序反转，故撤销 score 驱动 sizing、恢复固定 5 shares。ECMWF fallback 污染窗口需剔除/分层；market-anchored residual / overshoot-aware 只作独立 shadow | [1]-[4] pre_predict / forecast_quality · [selector refinement](analysis/2026-07/2026-07-02-low-price-yes-lottery-selector-refinement-v1.md) · [HeadA refinement](analysis/2026-07/2026-07-04-low-price-yes-heada-refinement-v1.md) · [live audit](analysis/2026-07/2026-07-10-low-price-yes-heada-live-audit-v1.md) · [strategy reset](analysis/2026-07/2026-07-14-strategy-search-reset-v1.md) |
...
| current_yes_heat_death_physical_v1（H1 late-carry / H2 early dislocation 双头） | 13-17 本地时段 heat-death 确认（decline>=0.5、高点成熟 60m、path 不升温、forecast peak 已过）后买 current YES 或 d1 NO；按入场价拆两头：ask>=0.95 = late-carry premium，ask<=0.93 = Busan 型 early dislocation | **`shadow + live`（10-share dual-head probe：H1 [0.95,0.99] / H2 [0.50,0.93] 两独立实例，`--live --confirm-live`，未确认；2026-07-15 拆分,原单实例零成交退役）** | 2026-07-15：发现 factory 内嵌旧污染 peak clock（600/976 city-date 不一致），已用 single-runs D-1 12z 重建 factory 并重跑：H1 holdout 从 15 行缩到 7 行/6 天，current YES +5.8% CI[+1.2,+18.1] 但 significance=FAIL_LOW_SAMPLE（预注册地板 30 行/12 天），相对同价 base-fade 超额 +0.4% CI 跨 0（物理增量仍不存在）；旧 15 行 +2.1% 证据作废。H2 扩窗到 7/07-14（8 结算日、anchor 剔除、proxy 键控修复）后 minute-price proxy base current YES +0.31%（+2c 后 -1.13%），无正边际，可执行首信号盘口证据待 forward。两头晋升判据已预注册冻结，probe fill 按入场价 regime 归属；**注意 H1 历史绝对 ROI 依赖的 factory peak clock 在 2026-07-15 被证实内嵌旧污染 backfill（600/976 city-date 与 single-runs 重建不一致），干净 factory 重跑前 H1 绝对 ROI 视为 source-contaminated** | [1]-[4] reheat_risk · [backtest](analysis/2026-07/2026-07-14-current-yes-heat-death-physical-backtest-v1.md) · [early replay](analysis/2026-07/2026-07-14-heat-death-early-event-replay-v1.md) · [shadow](analysis/2026-07/2026-07-14-current-yes-heat-death-physical-shadow-v1.md) · [晋升判据预注册](analysis/2026-07/2026-07-15-heat-death-live-promotion-preregistration-v1.md) |
...
| metar_cross_prev_no / fast_source_prev_no_trial_v1 | 快源先看到跨档后买 previous exact bracket NO；alpha 假设是 source→settlement-facing observation 的时间差 | **`running probe / rejected-as-main-strategy`** | 2026-07-15 Mac 可见带 `--live --confirm-live` 的 probe 进程，但证据仍只有少量独立结算日，且经历 FOK share-cap 事故；“进程在跑”不改变 collector-first、不得扩 live 的 verdict | [0]-[4] latency_arb · [denominator audit](analysis/2026-07/2026-07-14-source-event-denominator-audit-v2.md) · [share-cap incident](analysis/2026-07/2026-07-14-fast-source-share-cap-incident-v1.md) · [strategy reset](analysis/2026-07/2026-07-14-strategy-search-reset-v1.md) |
| hko_official_tminus1_no_live_v1 | 香港天文台 HKO 官方站已出现 floor(T) 后，只买最高温 `T-1 NO`；不消费 VHHH/METAR，不买 current YES | **`runner active / live not evidenced`** | 2026-07-15 可见 Mac loop，但 command line 无 `--live --confirm-live`；因此只记 active collector/paper runner，不能写成真实 live。后续 tiny probe 仍需部署确认与 raw/exchange order 证据 | [0]-[2] latency_arb · [HK readiness](analysis/2026-07/2026-07-11-tokyo-hk-fast-source-live-readiness-v1.md) · [share-cap incident](analysis/2026-07/2026-07-14-fast-source-share-cap-incident-v1.md) |
...
| d1_yes_high_mid_v1（instance `d1_yes_high_mid_shadow_v1`） | 全分母校准曲线发现的唯一可能穿 taker 摩擦格子：d1（running max 上一档 exact bracket）yes-mid≥0.80 时买 d1 YES（taker `1-no_bid`，均价~0.92）；语义=市场已确认强升温日会正好完成 +1 档时仍略保守 | **`shadow`（zero-notional，已接血缘/监控，未 live）** | all-rows 296 rows/48 dates fee ROI +2.18% CI[+0.04%,+4.47%]；诚实分母（first row/city-date）+2.24% CI 跨 0；forward(6/21+) 15 dates +6.62% CI[+3.47%,+9.89%] 但 train 只 +0.73%。发现路径含 ~50 格扫描的事后选择；亏损集中 plateau/overshoot 城市（Taipei/Madrid/Lucknow 等），不预注册城市剔除；NO bid 深度未验证。runner `scripts/ops/d1_yes_high_mid_shadow_v1.py`（复用 factory d1 anchoring），已注册 strategy_def `d1_yes_high_mid` / instance / config / shadow_queue + 监控 WatchSpec；依赖全 ladder 采集，`snapshot-targeted` 下 narrow。促升走既有 10 独立结算日 + 双 CI gate | [1]-[2] market_structure_edge · [strategy doc](analysis/2026-07/2026-07-15-d1-yes-high-mid-strategy-v1.md) · [calibration curve](analysis/2026-07/2026-07-15-market-calibration-curve-v1.md) |
