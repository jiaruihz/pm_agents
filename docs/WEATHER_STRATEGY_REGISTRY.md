...
| low_price_yes_lottery_tiny_live_v1 | refined low-price BUY_YES longshot：`edge>=0.20`、ask `0.05..0.20`、`dist>0`、每 city-date 一笔，fresh-book maker-first；2026-07-04 起 live sizing=`price_tier_6_8_10_shares`（5-8c→6 shares，8-14c→8，14-20c→10），并记录 fixed-cash / fixed-8-share / quality-tier shadow sizing、station-basis / p_cal / bracket-distance / book-state telemetry | **`live`（tiny-live forward probe）** | 用户 2026-07-02 批准微仓前向取证；2026-07-04 按 HeadA refinement 把 live 仓位从 fixed `$0.8/order` 改为低自由度 price-tier sizing，但这只是 tiny-live 执行几何调整，不是 confirmed alpha 或 size-up。核心风险仍是少数尾部命中驱动与成交质量；只按独立 strategy family 记录，不接 regime-routed NO runner；telemetry 只作 fresh-forward 验证 | [1]-[4] pre_predict / forecast_quality · [selector refinement](analysis/2026-07/2026-07-02-low-price-yes-lottery-selector-refinement-v1.md) · [HeadA refinement](analysis/2026-07/2026-07-04-low-price-yes-heada-refinement-v1.md) |
