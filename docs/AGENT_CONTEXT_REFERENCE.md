# PM Agent — 完整历史上下文参考（按需读取）

> 本文件由原根目录 `AGENTS.md` 迁入，仅用于历史事故、生产拓扑或领域细节确实相关时按需检索。
> 默认入口是根目录 `AGENTS.md`；不要在每个任务开始时加载本文件全文。
...
- **代理入口与路由也只有一个 owner**：所有天气生产调用只经过共享 `weather_market_proxy` 接口；未传 `route_key` 一律走 `default`，不得把路由 profile 下沉到每个策略或逐进程改代理。当前 weather `default` 与公共不可逆执行 handoff 的 `stable` 都解析到同一 Clash 进程的 `7896 / PM-STABLE`，由 TAG 本机 `7890` 优先、Allblue 兜底；`allblue` 命名备选为 `7897`，仅供 control-plane 维护 fallback selector，不作天气 consumer 默认入口。切上游不修改 consumer 参数；Allblue selector 连续失败时只由 control-plane 有界切换已有候选并做 Gamma/CLOB 与 Polymarket geoblock 后验，失败恢复原节点并写审计。controller 只通过 Clash 既有 Unix socket 读写 route/group，不启用 TCP external controller、不创建第二套 secret、不恢复历史 weather failover LaunchAgent；系统/Codex 与加密流量不受该 weather 合同控制。主机级 TAG 节点选择只能由独立 `scripts/ops/tag_proxy_route_ctl.py` 管理：复用 TAG 已有本机 controller/secret，只在 `1x` 候选间经连续失败、冷却、OpenAI 后验和失败回滚后切换，不修改系统代理端口、weather route 或 consumer 参数；但 Polymarket `blocked=true` 是不可逆执行故障，必须跳过连续失败与冷却立即切换，候选只有同时通过 OpenAI 和 `trading_allowed=true` 才可接受。其 LaunchAgent 只能请求 bounded one-shot。所有天气上游失败时显式降级，禁止静默 direct。
...
- **可用性监督必须独立于被监督故障域且只有一个 scheduler**：`production_reliability_supervisor.py` 是 Mac 内置盘上的 bounded one-shot，由独立 LaunchAgent 周期请求；它读取 weather controller、production UUID/容量、weather 网络、只读 execution semantic probe 和 crypto runtime registry/data freshness，不在 JRS tmux 内常驻，也不维护第二份 desired state。`weather_execution_semantic_health.py` 只提供 probe library/one-shot，不得自行 loop、通知或恢复；旧 `weather_runtime_monitor`、live patrol/doctor 不得恢复。自动动作只能调用既有 controller/reconciler，且仅限 weather `recovery_policy=safe && expected_live=false` 与 crypto research/shadow/dry-run；JRS/TCC、UUID/容量、全网中断、guarded live 一律 fail closed、记录影响窗口并通知。机器断电或整机断网只能由配置了 `PRODUCTION_RELIABILITY_HEARTBEAT_URL` 的外部 dead-man switch 发现，不能把本机 Telegram 当作该场景的保证。
