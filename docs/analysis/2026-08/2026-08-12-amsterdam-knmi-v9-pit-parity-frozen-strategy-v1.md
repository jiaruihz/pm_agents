...
状态：`V7 production evidence invalidated by feature-contract bug / V9 zero-notional clean forward / no live capital`
...
V9 的 `2pp` policy 是在 post-freeze development 后锁定，clean forward 从 Amsterdam local 2026-08-12 00:00（UTC 2026-08-11 22:00）开始；以后不再用 8 月 1–11 日调该 policy。
...
下一次策略判断只看 2026-08-12 起新增的 settled target dates；30 dates 前只报告累计 funnel 与诊断，不用同一 forward 标签继续换阈值。
