...
9. Amsterdam/Helsinki/Tokyo pooled `ΔP_weather × ladder transport` 已按15/30/60s冻结输入与M0–M3比较；旧历史联合full-ladder rows仍为0，所以尚无新回测。缺口已转为正在运行的共享forward collection：不再用binary current-bracket或3-rung hot strip替代；先积累三城守恒整梯probability + full-book路径，不创建策略、不改live。
