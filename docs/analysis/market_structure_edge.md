...
`ladder_mass_transport_v1` is separately `BRANCH_EXHAUSTED` at the primary probability/markout gate. Development was fixed to 2026-07-11..21 and selected `M3 weather-response-lag`, Ridge alpha 10, 60m from 15 M0–M3/static-kink × alpha arms; no arm survives Holm correction. The single-open frozen historical validation (2026-07-22..28, not untouched because kink research had seen those dates) contains 23,887 rung rows / 3,174 snapshots / 7 dates / 47 cities. M3 relative-markout MSE is `0.00111178` versus M0 `0.00111411`, but paired delta `-0.00000233` has target-date CI `[-0.00000547,+0.00000051]`; versus M1 and static kink the CIs also cross zero. Directional and settlement heads improve, but the required continuous markout gate fails: only 4/7 dates improve, pooled and leave-one-city-out results improve in 21/47 cities, and liquidity regimes have mixed signs. Execution styles and break-even are therefore `NOT_RUN` by contract, no strategy expression is frozen, and no zero-notional shadow is started. M4 is blocked because this historical window has no deterministic WS reconstruction/parity or trade prints, but that does not weaken the completed M0–M3 rejection. Artifact: `/Volumes/jrs-archive/pm_agents/research/artifact_store/market_structure_edge/ladder_mass_transport_20260809`.
