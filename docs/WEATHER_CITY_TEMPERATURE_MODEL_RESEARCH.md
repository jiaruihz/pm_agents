...
Updated: 2026-08-12 Tokyo first pre-cross bounded market posterior
...
|---|---|---|---|
| Amsterdam | WCIR Amsterdam adapter；V9 weather head + independent `amsterdam_knmi_market_offset_probability_v2`；coefficient-one direct current-NO market prior，YES/NO exact-bracket expression | standalone V9未胜market后改为只学习weather/path log-odds correction。修正`1-q_current`错分母与validation refit泄漏后，6–7月expanding OOF为6,136 rows/57日：posterior Brier/logloss `0.04056/0.13588`，market `0.04718/0.15212`，delta CI仍轻微跨0。固定`:10/:40`、10–16点、2pp、5-share sampled-price policy为98笔/55日、78胜、fee proxy ROI `+11.85%` CI `[+0.61%,+23.05%]`；同rows market favorite ROI `-5.95%`，paired PnL uplift CI全正。7/30–8/11 collector t0 narrow window为7/7、ROI `+31.49%`，但方向与market favorite相同，不能冒充额外alpha | 新head为zero-notional shadow candidate，artifact已冻结、WCIR adapter/config code-ready，未重启生产、不动live CrossNO。先积累clean-forward executable books/settlements；要求proper-score delta CI全负且fee PnL uplift CI为正再考虑升阶。adapter已修physical current低于`or below`档时的hard-floor expression anchor；单边book midpoint缺失时fail-closed · [V9 + market-offset update](analysis/2026-08/2026-08-12-amsterdam-knmi-v9-pit-parity-frozen-strategy-v1.md) |
...
| Helsinki | bounded market-prior residual：FMI first-seen开仓、METAR仅校正/退出；5-share YES/NO symmetric taker | 天气基座2025 OOF为51,451 rows/365日，冻结2026 no-forecast fallback Brier/logloss=`0.08277/0.28370`、AUC `0.95610`，不是随机模型；结构弱点集中于plateau（Brier `0.14021`）和14–18（`0.12107`）。概率呈压缩型S形而非全局过度自信，matching calibration在2026未改善。盘口residual仅362 checkpoints/9日，9种日期互换选出的cap从`0.05`漂至`1.50`；复杂`c015+Platt`在开发折8/9胜，但8月回放27笔23胜仍ROI `-4.42%`，属于过拟合。incumbent `c=0.15`的8/2–11回放为9笔6胜、ROI `+30.31%`，但CI跨0 | 保持当前zero-notional artifact，不用已看过的8月窗口换模型；下一版只做forecast-availability分头与peak-clock/path-transition连续特征。先让冻结5-share taker在新settled日期同分母为正，maker只作为fill/queue/adverse-selection验证后的收益放大器；不加价格/小时/坏例hard gate · [strategy + robustness audit](analysis/2026-08/2026-08-12-helsinki-bounded-market-residual-strategy-v1.md) |
| Tokyo | first pre-cross source state + bounded market-confirmed posterior；每档只在JMA首次到`current+0.3/+0.4°C`且尚未native cross时判断一次，market NO<0.5不反转，≥0.5时用开发窗冻结的`2×logit`强化 | 修复development exact-clock漏数和bracket-relative feature错anchor后，scope为63,384历史feature rows/819日→2,244首次pre-cross candidates/737日；7月development 124 expressions中9 candidates/6日，8/1–11 strict raw-exact 554 expressions中34 candidates/11日。source-only physical模型在market overlap仍输market；bounded posterior在reused audit Brier/logloss=`0.04961/0.14541`，market=`0.05307/0.17065`，Brier delta CI `[-0.00792,+0.00081]`。首次正净edge为29笔28胜，5-share fee后PnL`+$3.1543`、ROI`+2.30%`，date CI `[-1.19%,+5.65%]` | 状态`shadow_candidate / live gates FAIL`；8/1–11因方向形成前已查看只算reused audit。冻结8/12+离线zero-notional，首日当前3 exact expressions、0 pre-cross signal；不接intent/order/fill，不按8/11唯一terminal loss补hard filter · [pre-cross report](analysis/2026-08/2026-08-12-tokyo-pre-cross-market-sharpening-v1.md) · [superseded broad blend](analysis/2026-08/2026-08-12-tokyo-market-weather-posterior-v1.md) |
...
**Helsinki 2026-08-12 更新**：`helsinki_bounded_market_residual_c015_symmetric_v1`已完成反复时间切分、校准和复杂度压力测试。天气基座有真实预测力，但plateau/午后峰值转换偏弱；盘口cap选择不稳定，复杂calibrator出现“27笔23胜但ROI `-4.42%`”的典型过拟合。当前`c=0.15`在8/2–11 retrospective PIT为9笔6胜、PnL`+$6.98`、ROI `+30.31%`，但proper-score和ROI CI均跨0，因此不替换artifact、不升live。正式目标是先让冻结5-share taker在新日期稳定小正，再评估maker是否放大收益。详见 [strategy report](analysis/2026-08/2026-08-12-helsinki-bounded-market-residual-strategy-v1.md)。
