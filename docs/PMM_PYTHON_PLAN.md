...
- 只撤销“偏离过大”的订单
- 只新增“需要挂单”的订单
- **队列优先（Queue Priority / Deadband）**：
  - 订单簿遵循“价格优先、时间优先”
  - 如果新旧价格相同或差异极小，**不要撤单重挂**
  - 只有当“价格变动带来的收益” > “失去排队位置的成本”时才更新
  - 可用 `deadband` 参数控制：`|new_price - old_price| < ε` 则保留
...
- Python PMM 进程只需要调用：
  ...
  - `/balance`
  - `/ctf/merge`
- **建议优先使用 WebSocket 订阅代替轮询**：
  - 订单簿（orderbook）与成交（fills）事件尽量走推送
  - 只有在 WS 断连或回补时才用 REST 轮询
