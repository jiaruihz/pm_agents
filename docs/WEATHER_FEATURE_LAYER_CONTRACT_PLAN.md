...
Phase 5A/5B adds shared exact-bracket settlement geometry, symmetric bid/ask
book-state fields, and side-aware YES/NO quote helpers with tmax P0/P3 and
...
- [done 2026-07-06 Phase 5B] Add side-aware BUY_YES/BUY_NO quote helpers for Range RV and TP/stop replay fixtures.
...
- HeadA mechanism overlay candidate rows match old bracket-distance and book-state fields. Bracket-distance is already delegated through `low_price_yes_tail_telemetry.py`; fresh live quote fields remain runner-local until a consumer replay is approved.
- Bid/ask ladder fields round-trip on Range RV and TP/stop fixture rows. Done for side cost/spread parity with Range RV scanner/variant helpers and YES-book BUY_YES/BUY_NO complement mapping.
