...
```bash
pkill -f "src.strategies.pmm.main"
```
...
```bash
set -a; source .env; set +a
.venv/bin/python scripts/ops/telegram_ping.py --text "PMM paper telegram test"
```
...
```bash
set -a; source .env; set +a
.venv/bin/python -m src.interfaces.web.strategy_dashboard_server --host 127.0.0.1 --port 8011 --artifacts-dir src/strategies/pmm/backtest/.artifacts --runtime-dir runtime
```
