...
- `utils/` 不应依赖其他 `src.strategies.pmm` 子包
...
python scripts/python/pmm_orderbook_capture.py capture \
  ...
  --duration 180 \
  --interval 1.0 \
  --out-scenario src/strategies/pmm/backtest/.artifacts/recorded/sample.json

# 把 jsonl 转成可直接 replay 的 scenario
python scripts/python/pmm_orderbook_capture.py convert \
  --jsonl src/strategies/pmm/backtest/.artifacts/recorded/sample.jsonl.gz \
  --out-scenario src/strategies/pmm/backtest/.artifacts/recorded/sample_converted.json
```
...
- 产物分层：
  - `src/strategies/pmm/backtest/.artifacts/`：运行产物，默认不入库
  - `src/strategies/pmm/backtest/metadata/`：汇总元数据，可入库
