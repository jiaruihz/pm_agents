...
- **可用性监督必须独立于被监督故障域**：`production_reliability_supervisor.py` 是 Mac 内置盘上的 bounded one-shot，由独立 LaunchAgent 周期请求；它读取 weather controller、production UUID/容量、weather 网络探针和 crypto runtime registry/data freshness，不在 JRS tmux 内常驻，也不维护第二份 desired state。自动动作只能调用既有 controller/reconciler，且仅限 weather `recovery_policy=safe && expected_live=false` 与 crypto research/shadow/dry-run；JRS/TCC、UUID/容量、全网中断、guarded live 一律 fail closed、记录影响窗口并通知。机器断电或整机断网只能由配置了 `PRODUCTION_RELIABILITY_HEARTBEAT_URL` 的外部 dead-man switch 发现，不能把本机 Telegram 当作该场景的保证。
