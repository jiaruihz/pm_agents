...
- **存储和写入目标也由 production contract 管**：current raw root、canonical DB、兼容入口、active live order journal 与 health artifact 只能从 `production.yaml`/共享 loader 解析，不在 health、refresh、analysis 脚本维护第二份策略名或路径清单，也不静默 fallback 到 `weather-predict`、N100 mirror 或 repo-local 历史 runtime。大型研究机器产物统一进入 `production.yaml.research_artifact_root` 的 JRS 内容寻址对象库；仓库 `docs/` 只保留结论、紧凑 metadata、权威 case evidence 与仍被活跃代码消费的显式例外，恢复使用 `scripts/ops/weather_research_artifact_ctl.py restore`。一个 mutable target 只能有一个 owner；`latest.json` 是可覆盖 cache，历史证据用 append-only JSONL 且必须带 `schema_version`、稳定 event/order identity、event/observed/ingested 时钟和 writer/build identity。用 `scripts/ops/weather_storage_identity_audit.py` 区分同 inode alias、历史只读副本和危险的独立可写正本；发现 distinct `weather.db`、共享 live journal 或 schema/shape drift 时先修 P0，不凭文件名猜哪份是真的。
