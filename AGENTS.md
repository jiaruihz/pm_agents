...
- **生产代码按 release identity 管理**：`production.yaml.production_releases` 是 checkout root + full `expected_repo_sha` 的唯一映射，业务 runtime 只引用 `release_id`，不得各自重复路径或跟随 branch HEAD。部署先提交代码、显式推进 release pin，再由 controller 重载对应 runtime；manifest 的 `production_release_checkout_sha_mismatch` 或 `process_release_sha_mismatch` 均表示 desired/loaded identity 未收口，live 进程命中时按 critical 处理。分支名、checkout 干净或 session 存在都不能替代 loaded SHA 验证。
