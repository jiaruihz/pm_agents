...
- **代理入口与路由也只有一个 owner**：所有天气生产调用只经过共享 `weather_market_proxy` 接口；未传 `route_key` 一律走 `default`，不得把路由 profile 下沉到每个策略或逐进程改代理。当前 weather `default` 与公共不可逆执行 handoff 的 `stable` 都解析到同一 Clash 进程的 `7896 / PM-STABLE`，由 TAG 本机 `7890` 优先、Allblue 兜底；`allblue` 命名备选为 `7897`，仅供 control-plane 维护 fallback selector，不作天气 consumer 默认入口。切上游不修改 consumer 参数；Allblue selector 连续失败时只由 control-plane 有界切换已有候选并做 Gamma/CLOB 后验，失败恢复原节点并写审计。controller 只通过 Clash 既有 Unix socket 读写 route/group，不启用 TCP external controller、不创建第二套 secret、不恢复历史 weather failover LaunchAgent；系统/Codex 与加密流量不受该 weather 合同控制。主机级 TAG 节点选择只能由独立 `scripts/ops/tag_proxy_route_ctl.py` 管理：复用 TAG 已有本机 controller/secret，只在 `1x` 候选间经连续失败、冷却、OpenAI 后验和失败回滚后切换，不修改系统代理端口、weather route 或 consumer 参数；其 LaunchAgent 只能请求 bounded one-shot。所有天气上游失败时显式降级，禁止静默 direct。
